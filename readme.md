...
# Note
```txt
Because Mongoose automatically handles queries through its global connection singleton, the caller doesn't need to do anything with the return value of dbConnect(). Returning Promise<void> keeps it simple.
...
```txt
  "use client"
  is something which says this typescirpt will not go directly to server 
...
so jaha bhi use vo hooks hote hai jo methods hote hai
aut unko data chahiye hota hai

https://youtu.be/G9SKBXx_P-M?t=355
```
# notes
```
...
const handelDeleteConfirm = async () =>{
    ...
    _id >> this is "_" commany used in mongodb
```
# notes
```
export async function POST(req: Request, { params }: { params: { messageId: string } }) {
  console.log(
    chalk.blue("[API] > "),
    "POST /api/delete-message/"+params.messageId+" request received"
  );
...
# notes
```
https://youtu.be/28-fJmm_ONQ?t=150

code consitency

as toggle switch is also a form.

```
# notes
```
https://chatgpt.com/s/t_6a5123f8b7708191afd07950a0e94e62

'use client'

Does this component need to respond to user interactions or use browser APIs?
...
const fetchMessages = useCallback(async (refresh: boolean = false) => {
  ...
  default value on args
```
# notes
```
Why import type exists

Suppose you write:

import { User } from "./types";

If User is only an interface,

export interface User {
  name: string;
}

then JavaScript tries to execute:

import { User } from "./types";

But interfaces don't exist after compilation.
...
"This is only for type checking. Remove it from the generated JavaScript."
```
